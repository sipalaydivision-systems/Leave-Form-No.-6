...
// Merge two employee accounts into one (keeps primaryEmail, transfers data from duplicateEmail)
app.post('/api/merge-employees', (req, res) => {
    try {
        const { primaryEmail, duplicateEmail, mergedBy } = req.body;

        if (!primaryEmail || !duplicateEmail) {
            return res.status(400).json({ success: false, error: 'Both primaryEmail and duplicateEmail are required' });
        }

        if (primaryEmail === duplicateEmail) {
            return res.status(400).json({ success: false, error: 'Primary and duplicate emails must be different' });
        }

        console.log(`[MERGE] Starting merge: "${duplicateEmail}" → "${primaryEmail}" (by ${mergedBy})`);

        let mergeActions = [];

        // 1. Merge leave cards: transfer transactions from duplicate to primary
        let leavecards = readJSON(leavecardsFile);
        const primaryLC = leavecards.find(lc => lc.email === primaryEmail || lc.employeeId === primaryEmail);
        const duplicateLC = leavecards.find(lc => lc.email === duplicateEmail || lc.employeeId === duplicateEmail);

        if (duplicateLC) {
            if (primaryLC) {
                // Merge transactions from duplicate into primary
                if (duplicateLC.transactions && duplicateLC.transactions.length > 0) {
                    primaryLC.transactions = primaryLC.transactions || [];
                    primaryLC.transactions.push(...duplicateLC.transactions);
                    mergeActions.push(`Merged ${duplicateLC.transactions.length} leave transactions`);
                }
                // Merge leave usage history
                if (duplicateLC.leaveUsageHistory && duplicateLC.leaveUsageHistory.length > 0) {
                    primaryLC.leaveUsageHistory = primaryLC.leaveUsageHistory || [];
                    primaryLC.leaveUsageHistory.push(...duplicateLC.leaveUsageHistory);
                    mergeActions.push(`Merged ${duplicateLC.leaveUsageHistory.length} leave usage history entries`);
                }
                // Use higher earned values
                if ((duplicateLC.vacationLeaveEarned || 0) > (primaryLC.vacationLeaveEarned || 0)) {
                    primaryLC.vacationLeaveEarned = duplicateLC.vacationLeaveEarned;
                }
                if ((duplicateLC.sickLeaveEarned || 0) > (primaryLC.sickLeaveEarned || 0)) {
                    primaryLC.sickLeaveEarned = duplicateLC.sickLeaveEarned;
                }
                primaryLC.updatedAt = new Date().toISOString();
                // Remove the duplicate leave card
                leavecards = leavecards.filter(lc => lc !== duplicateLC);
                mergeActions.push('Removed duplicate leave card');
            } else {
                // No primary leave card — just re-link the duplicate's card to the primary email
                duplicateLC.email = primaryEmail;
                duplicateLC.employeeId = primaryEmail;
                duplicateLC.updatedAt = new Date().toISOString();
                mergeActions.push('Re-linked leave card to primary email');
            }
            writeJSON(leavecardsFile, leavecards);
        }

        // 2. Merge CTO records: re-link all duplicate's CTO records to primary
        ensureFile(ctoRecordsFile);
        let ctoRecords = readJSON(ctoRecordsFile);
        let ctoMergeCount = 0;
        ctoRecords.forEach(cto => {
            if (cto.employeeId === duplicateEmail || cto.email === duplicateEmail) {
                cto.employeeId = primaryEmail;
                cto.email = primaryEmail;
                ctoMergeCount++;
            }
        });
        if (ctoMergeCount > 0) {
            writeJSON(ctoRecordsFile, ctoRecords);
            mergeActions.push(`Re-linked ${ctoMergeCount} CTO records`);
        }

        // 3. Merge leave applications: re-link all duplicate's applications to primary
        let applications = readJSONArray(applicationsFile);
        let appMergeCount = 0;
        applications.forEach(app => {
            if (app.employeeEmail === duplicateEmail || app.email === duplicateEmail) {
                app.employeeEmail = primaryEmail;
                if (app.email === duplicateEmail) app.email = primaryEmail;
                appMergeCount++;
            }
        });
        if (appMergeCount > 0) {
            writeJSON(applicationsFile, applications);
            mergeActions.push(`Re-linked ${appMergeCount} leave applications`);
        }

        // 4. Remove the duplicate user account from users.json
        let users = readJSON(usersFile);
        const dupUserIndex = users.findIndex(u => u.email === duplicateEmail);
        let dupUserName = null;
        if (dupUserIndex !== -1) {
            dupUserName = users[dupUserIndex].name || users[dupUserIndex].fullName;
            users.splice(dupUserIndex, 1);
            writeJSON(usersFile, users);
            mergeActions.push(`Removed duplicate user account "${duplicateEmail}"`);
        }

        // 5. Remove from employees.json too
        let employees = readJSON(employeesFile);
        const dupEmpIndex = employees.findIndex(e => e.email === duplicateEmail);
        if (dupEmpIndex !== -1) {
            employees.splice(dupEmpIndex, 1);
            writeJSON(employeesFile, employees);
            mergeActions.push('Removed duplicate from employees list');
        }

        // 6. Remove from pending registrations if exists
        let pendingRegs = readJSON(pendingRegistrationsFile);
        const dupRegIndex = pendingRegs.findIndex(r => r.email === duplicateEmail && r.portal === 'employee');
        if (dupRegIndex !== -1) {
            pendingRegs.splice(dupRegIndex, 1);
            writeJSON(pendingRegistrationsFile, pendingRegs);
            mergeActions.push('Removed duplicate pending registration');
        }

        // Log the merge action
        logActivity('EMPLOYEE_MERGE', 'system', {
            primaryEmail,
            duplicateEmail,
            mergedBy: mergedBy || 'AO',
            actions: mergeActions,
            ip: getClientIp(req),
            userAgent: req.get('user-agent')
        });

        console.log(`[MERGE] Complete: "${duplicateEmail}" → "${primaryEmail}". Actions: ${mergeActions.join(', ')}`);

        res.json({
            success: true,
            message: `Successfully merged "${duplicateEmail}" into "${primaryEmail}"`,
            actions: mergeActions
        });
    } catch (error) {
...
app.get('/api/all-employees', (req, res) => {
    try {
        ...
        // Detect potential duplicates by normalized name similarity
        const duplicateGroups = [];
        const checked = new Set();
        for (let i = 0; i < employees.length; i++) {
            if (checked.has(i)) continue;
            const nameA = normalizeNameForMatching(employees[i].name || '');
            const group = [employees[i].email];
            for (let j = i + 1; j < employees.length; j++) {
                if (checked.has(j)) continue;
                const nameB = normalizeNameForMatching(employees[j].name || '');
                // Check if names share significant words (at least 2 words in common)
                const wordsA = nameA.split(/\s+/).filter(w => w.length > 1);
                const wordsB = nameB.split(/\s+/).filter(w => w.length > 1);
                const commonWords = wordsA.filter(w => wordsB.includes(w));
                if (commonWords.length >= 2 && employees[i].email !== employees[j].email) {
                    group.push(employees[j].email);
                    checked.add(j);
                }
            }
            if (group.length > 1) {
        ...
        // Mark employees that are part of a duplicate group
        if (duplicateGroups.length > 0) {
            employees.forEach(emp => {
                const group = duplicateGroups.find(g => g.includes(emp.email));
                if (group) {
                    emp.duplicateGroup = group;
                }
            });
        }
        
        res.json({ success: true, employees: employees, duplicateGroups: duplicateGroups });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
});
