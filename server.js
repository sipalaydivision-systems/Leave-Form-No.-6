...
app.post('/api/admin-officer-register', apiRateLimiter, createAdminRegisterHandler({
    ...
    excludePortals: ['aov', 'user', 'hr', 'asds', 'sds'],
...
app.post('/api/asds-register', apiRateLimiter, createAdminRegisterHandler({
    ...
    excludePortals: ['asds', 'user', 'hr', 'aov', 'sds']
}));
...
app.post('/api/sds-register', apiRateLimiter, createAdminRegisterHandler({
    ...
    excludePortals: ['sds', 'user', 'hr', 'aov', 'asds'],
...
// Update HR Profile
app.post('/api/update-aov-profile', requireAuth('aov'), createProfileUpdateHandler({
...
app.post('/api/delete-all-data', requireAuth('it'), loginRateLimiter, (req, res) => {
    try {
        ...
        const dataFilesToClear = [
            ...
            hrUsersFile,                // AO users
            aovUsersFile,                // HR users
...
app.get('/api/pending-applications/:portal', requireAuth('hr', 'aov', 'asds', 'sds', 'it'), (req, res) => {
    try {
...
app.get('/api/approved-applications/:portal', requireAuth('hr', 'aov', 'asds', 'sds', 'it'), (req, res) => {
    try {
        const portal = req.params.portal.toUpperCase();
        const applications = readJSONArray(applicationsFile);
...
app.get('/api/hr-approved-applications', requireAuth('aov', 'asds', 'sds', 'it'), (req, res) => {
    try {
        const applications = readJSONArray(applicationsFile);
...
app.get('/api/all-users', requireAuth('hr', 'aov', 'asds', 'sds', 'it'), (req, res) => {
    try {
        const users = readJSON(usersFile);
...
app.get('/api/all-applications', requireAuth('hr', 'aov', 'asds', 'sds', 'it'), (req, res) => {
    try {
        let applications = readJSONArray(applicationsFile);
...
app.get('/api/leave-calendar', requireAuth('hr', 'aov', 'asds', 'sds', 'it'), (req, res) => {
    try {
...
app.get('/api/all-employees', requireAuth('hr', 'aov', 'it'), (req, res) => {
    try {
        const users = readJSON(usersFile);
        const leavecards = readJSON(leavecardsFile);
...
app.get('/api/portal-applications/:portal', requireAuth('hr', 'aov', 'asds', 'sds', 'it'), (req, res) => {
    try {
        const portal = req.params.portal.toUpperCase();
        const applications = readJSONArray(applicationsFile);
...
app.post('/api/update-leave-credits', requireAuth('hr', 'it'), (req, res) => {
    try {
        const { 
            applicationId, 
            employeeId,
...
app.post('/api/approve-leave', requireAuth('aov', 'hr', 'asds', 'sds'), (req, res) => {
    try {
        ...
        let stepSignature = '';
        if (action === 'approved') {
            if (currentApprover === 'AOV' && authorizedOfficerSignature) stepSignature = authorizedOfficerSignature;
...
app.get('/api/cto-records', requireAuth(), (req, res) => {
    try {
        ...
        // AO school-based filtering
        if (employeeId && !isHrAccessAllowed(req, employeeId)) {
            return res.status(403).json({ success: false, error: 'Access denied. This employee is not from your school.' });
        }
        let ctoRecords = readJSON(ctoRecordsFile);
...
app.post('/api/update-cto-records', requireAuth('hr', 'it'), (req, res) => {
    try {
...
app.put('/api/cto-records/:recordId', requireAuth('hr', 'it'), (req, res) => {
    try {
        ...
        // AO school-based filtering
        if (!isHrAccessAllowed(req, ctoRecords[index].employeeId || ctoRecords[index].email)) {
            return res.status(403).json({ success: false, error: 'Access denied. This employee is not from your school.' });
        }
...
app.delete('/api/cto-records/:recordId', requireAuth('hr', 'it'), (req, res) => {
    try {
        const recordId = req.params.recordId;
        ...
        logActivity('CTO_RECORD_DELETED', 'hr', {
